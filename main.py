...
def resize_image(i):
    # dziele 480 przez faktyczna wielkosc obrazka, szerokosc mnoze przez 
    # wspolczynnik skalujacy, wyliczam szerokosc z proporcji oryginalnej,
...
def gray_image(i):
    # w opencv maja odwrotnie rgb -> bgr
    # przejscie do skali szarosci
    i = cv2.cvtColor(i, cv2.COLOR_BGR2GRAY)
    return i


# generates set of indexes: (t, s_j, s_k, p_j, p_k)
...
def haar_features_coordinates(hfs_indexes, s, p):  # generates coordinates of features within unit square
...
def haar_features(ii, hfs_coords_window, j0, k0):
    features = np.zeros(len(hfs_coords_window), "int32")
...
if __name__ == '__main__':
    ...
    t2 = time.time()
    print("time integral img: " + str(t2 - t1))
    ...
    t1 = time.time()
    ii_0 = integral_image_1(img)
    t2 = time.time()
    print("time cumsum: " + str(t2 - t1))

    # cv2.imshow("integral image", ii / np.max(ii))
    # cv2.waitKey(0)
    ...
    t1 = time.time()
    print(np.sum(img[j1: j2 + 1, k1: k2 + 1]))
    t2 = time.time()
    ...
    print("is equal two methods?:", np.array_equal(ii, ii_0))

    print(iou([0, 0, 4, 5], [0, 0, 4, 5]))
    path_fddb_root = './'
    print("FDDB READ SINGLE FOLD STARTED...")
    t1 = time.time()
    X, y = fddb_read_single_fold(path_fddb_root, "FDDB-folds/FDDB-fold-01-ellipseList.txt", 10, hfs_coords, verbose=False)
    t2 = time.time()
    print("FDDB READ SINGLE FOLD DONE IN " + str(t2 - t1) + ' s.')
    print(X.shape, y.shape)
    # print(iou([0, 0, 49, 49], [0, 0, 99, 99]))
    # print(iou([0, 0, 99, 99], [0, 0, 49, 49]))
    # print(iou([25, 25, 74, 74], [0, 0, 99, 99]))
