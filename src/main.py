...
def integral_image(i):
    ...
    ii = ii.astype(
        "int32")  # without this line uint32 was assumed and underflow errors were alarmed by delta(...) function
    return ii
...
def haar_features2(ii, hfs_coords_window_subset, j0, k0, n, fi, features):
    for i, hf_coords_window in enumerate(hfs_coords_window_subset):
        features[fi[i]] = haar_feature(ii, hf_coords_window, j0, k0)
    return features
    ...
    while line != "":
        ...
        for z in range(n_img_faces):
            ...
            if j0 < 0 or k0 < 0 or j0 + w - 1 >= i.shape[0] or k0 + w - 1 >= i.shape[1]:
                ...
                continue
            if (w / ii.shape[
                0] < 0.05):  # min relative size of positive window (smaller may lead to division by zero when white regions in haar features have no area)
                if verbose:
...
def detect(i, clf, hfs_coords_subset, n, fi, clf_threshold=0.0):
    ...
    for s in range(DETECTION_SCALES):
        ...
        for j in range(j_start, ii.shape[0] - w + 1, dj):
            for k in range(k_start, ii.shape[1] - w + 1, dk):
                n_windows_max += 1
        print("WINDOWS TO BE CHECKED: " + str(n_windows_max))
    ...
    for s in range(DETECTION_SCALES):
        ...
        for j in range(j_start, ii.shape[0] - w + 1, dj):
            for k in range(k_start, ii.shape[1] - w + 1, dk):
                ...
                #     print("PROGRESS: " + str(int(100 * n_windows / n_windows_max)) + "%")
    averaged_faces, weights = cv2.groupRectangles(faces, 1, 0.1)
    for it, face in enumerate(averaged_faces):
...
if __name__ == "__main__":
    ...
    # t1 = time.time()
    # X_train, y_train, X_test, y_test = fddb_data("C:/Users/mrsei/Desktop/UczenieMaszynowe2/64/data/originalPics/", hfs_coords, 10, n)
    # t2 = time.time()
    ...
    path_clf = path_clfs_root + "fddb_ada" + clf_description + ".pkl"
    ...
    B = 8
    clf_description = data_description + "_T_" + str(T) + "_MD_" + str(MD)
    path_clf = path_clfs_root + "fddb_real" + clf_description + ".pkl"
    ...
    #       i na jej podstawie możemy wybrać próg odpowiedni dla nas
    # zad 3 - wykonać (w pętli) procedurę detekcyjną na rzecz całych obrazów testowych i wyraportować dokładność
