...
# Comment: all int(...) operations removed, assuming they were performed earlier outside i.e. hf_coords_window
# already as ints
@jit(nopython=True)
...
# def haar_features_2(ii, hfs_coords_window_subset, j0, k0, n, fi=None, features):
#     for i, hf_coords_window in enumerate(hfs_coords_window_subset):
#         features[fi[i]] = haar_feature(ii, hf_coords_window, j0, k0)
#     return features
...
def detect(i, clf, hfs_coords_subset, n, fi, clf_threshold=0.0):
    ...
    for s in range(DETECTION_SCALES):
        ...
        hfs_coords_window_subset = np.array(list(map(lambda npa: npa.astype("int32"), hfs_coords_window_subset)), dtype=object)
        for j in range(j_start, ii.shape[0] - w + 1, dj):
            for k in range(k_start, ii.shape[1] - w + 1, dk):
                t1_extraction = time.time()
                features = haar_features(ii, hfs_coords_window_subset, j, k, n, fi)
                t2_extraction = time.time()
                t1_detection = time.time()
                decision = clf.decision_function(np.array([features]))[0]
                t2_detection = time.time()
                # print("EXTRACTION: " + str(t2_extraction - t1_extraction) + " s, DECISION: " + str(t2_detection - t1_detection) + "s")
                ...
                if n_windows % progress_step == 0:
                    print("PROGRESS: " + str(int(100 * n_windows / n_windows_max) / 100))

    t2 = time.time()
    total_time = t2 - t1
    print("TOTAL TIME: " + str(total_time) + " s.")
    time_per_window = total_time / n_windows
    print("TIME PER WINDOW: " + str(time_per_window) + " s.")
    return i_resized
...
if __name__ == "__main__":
    ...
    # t1 = time.time()
    # X_train, y_train, X_test, y_test = fddb_data("./", hfs_coords, 10, n)
    # t2 = time.time()
